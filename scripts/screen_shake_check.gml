...
if shaking == true                          // shake code, preprocessed before drawing, to avoid putting a load of calcs on the draw event
{
    if shake_count > 0                      // if there is still time to screen shake, then shake away!
    {
                                            // NOTE: make this decrement with time? divide it by shake_count/shake_count_max?
    ...
    else                                    // if we're out of shake time, stop shaking, reset shake values
    {
        ...
        shake_count = shake_count_max;      // resets the shake timer to a generic max. NOTE: actually i think it's this one causing the shake intensity problem
    }
}
